...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for dirname in root_path_dirs:
            ...
            type_representative_path = path.join(root_path, dirname, "type_representative.jpg")
            type_representative_response = requests.post(
                settings.IPFS_URL + '/add',
                files=dict(
                    file=open(type_representative_path, "rb")))
            ...
            type_representative_low_res_response = requests.post(
                settings.IPFS_URL + '/add',
                files=dict(
                    file=open(type_representative_low_res_path, "rb")))

            type_representative_hash = type_representative_response.json()['Hash']
            type_representative_low_res_hash = type_representative_low_res_response.json()['Hash']
            ...
            nft_type.representative_image_low_res_ipfs_uri = type_representative_low_res_hash

            nft_type.save()

            # Iterate NFTs
            # nft-<index>.jpg
            nft_dir = walk(path.join(root_path, dirname))
            (nft_dir_path, _, nft_file_names) = list(nft_dir)[0]
            ...
            for nft_index, nft_dict in enumerate(spec_file_marshalled["nfts"]):
                ...
                nft_image_response = requests.post(
                    settings.IPFS_URL + '/add',
                    files=dict(
                        file=open(nft_image_path, "rb")))

                nft_image_low_res_path = path.join(root_path, dirname, nft_low_res_file_name)
                nft_image_low_res_response = requests.post(
                    settings.IPFS_URL + '/add',
                    files=dict(
                        file=open(nft_image_low_res_path, "rb")))

                nft.image_ipfs_uri = nft_image_response.json()['Hash']
                nft.image_low_res_ipfs_uri = nft_image_low_res_response.json()['Hash']

                nft.save()
                self.stdout.write(self.style.SUCCESS(path.join(nft_dir_path, nft_file_name)))
